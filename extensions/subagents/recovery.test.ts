...
for (const profile of ["reviewer", "review"]) {
  for (const mode of ["native", "continuation"] as const) {
    test(`${profile} retains its recorded role and review scope on ${mode} recovery`, async () => {
      ...
      try {
        ...
        if (profile === "reviewer") {
          ...
          persistSnapshot({
            ...
            execution: {
              ...
              attempts: [
                { ...settled.execution.selected, outcome: "selected" },
              ],
            },
          });
        }
        ...
        assert.deepEqual(
          manager.view.get(snap.id)!.execution,
          recordedExecution,
        );
